...
use crate::handlers::{server, PromptHandler, ResourceHandler, ToolHandler};
use axum::{
...
impl McpServer {
    ...
    //         // Validate origin and protocol version
    //         if let Err(status) = McpServer::validate_origin(&headers) {
    //             error!(
    ...
    //         if let Err(status) = McpServer::validate_protocol_version(&headers) {
    //             let version = headers
    //                 .get("MCP-Protocol-Version")
    //                 .and_then(|h| h.to_str().ok())
    //                 .unwrap_or("none");
