...
async fn test_server_startup() {
    ...
    match response {
        Ok(resp) => {
            ...
            // tool names "echo" and "bf2042_weapons_by_category" don't care about order
            assert!(tools.iter().any(|t| t["name"] == "echo"));
            assert!(tools
                .iter()
                .any(|t| t["name"] == "bf2042_weapons_by_category"));
...
async fn test_list_resources() {
    ...
    match response {
        Ok(resp) => {
            ...
            let example_resource = resources
                .iter()
            ...
            assert_eq!(
                example_resource["description"],
                "An example in-memory resource"
            );
    ...
    match response {
        Ok(resp) => {
            ...
            assert_eq!(
                ...
                "A template for accessing items by ID"
            );
